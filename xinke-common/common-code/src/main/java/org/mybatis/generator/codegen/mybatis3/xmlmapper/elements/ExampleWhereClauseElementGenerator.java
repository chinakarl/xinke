/*
 * Copyright 2009 The Apache Software Foundation Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except in
...
public class ExampleWhereClauseElementGenerator extends AbstractXmlElementGenerator {
    ...
    public XmlElement getMiddleForEachElement(final IntrospectedColumn introspectedColumn) {
        final StringBuilder sb = new StringBuilder();
        String criteriaAttribute;
        boolean typeHandled;
        String typeHandlerString;
        if (introspectedColumn == null) {
            criteriaAttribute = "condition.criterions"; //$NON-NLS-1$
            typeHandled = false;
        ...
        final XmlElement middleForEachElement = new XmlElement("foreach"); //$NON-NLS-1$
        ...
        final XmlElement chooseElement = new XmlElement("choose"); //$NON-NLS-1$
        ...
        when.addElement(new TextElement("and ${criterion.condition}")); //$NON-NLS-1$
        final XmlElement innerForEach = new XmlElement("foreach"); //$NON-NLS-1$
        ...
        when = new XmlElement("when"); //$NON-NLS-1$
        when.addAttribute(new Attribute("test", "criterion.dateValue")); //$NON-NLS-1$ //$NON-NLS-2$
        sb.setLength(0);
        sb.append("and ${criterion.condition} #{criterion.value"); //$NON-NLS-1$
        if (typeHandled) {
            sb.append(typeHandlerString);
        }
        sb.append(",jdbcType=TIMESTAMP}");
        when.addElement(new TextElement(sb.toString()));
        chooseElement.addElement(when);

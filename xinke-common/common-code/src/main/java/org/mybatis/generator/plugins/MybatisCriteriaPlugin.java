...
public class MybatisCriteriaPlugin extends PluginAdapter {
    ...
    public boolean validate(final List<String> warnings) {
        ...
        final String criterias = context.getJavaClientGeneratorConfiguration().getProperty("criteria");
    ...
    public boolean clientGenerated(final Interface interfaze, final TopLevelClass topLevelClass, final IntrospectedTable introspectedTable) {
        ...
        final FullyQualifiedJavaType types = new FullyQualifiedJavaType("java.util.Map<java.lang.String, java.lang.Object>");
        final Parameter parameter = new Parameter(types, "condition", "@Param(\"condition\")");
        final Parameter parameter2 = new Parameter(criteria, "example");
        final Parameter parameter3 = new Parameter(criteria, "criteria", "@Param(\"example\")");
        ...
        for (int i = 0; i < methods.size(); i++) {
            ...
            if (method.getFormattedContent(0, true).contains("Example")) {
                ...
                if (size == 1) {
                    ...
                    method.addParameter(1, parameter3);
                }
                // 通用修改 2012/6/3
                // else if (size == 2) {
                // method.removeParameter(1);
    ...
    public boolean sqlMapDocumentGenerated(final Document document, final IntrospectedTable introspectedTable) {
        ...
        // final XmlElement Where_Clause = (XmlElement)
        // document.getRootElement().getElements().get(1);
        // // 移除第一个
        // Where_Clause.removeElement(0);
        //
        // final StringBuilder sb = new StringBuilder();
        //        final XmlElement dynamicElement = new XmlElement("trim"); //$NON-NLS-1$
        ...
        //
        // for (final IntrospectedColumn introspectedColumn :
        // introspectedTable.getAllColumns()) {
        //            final XmlElement isNotNullElement = new XmlElement("if"); //$NON-NLS-1$
        // final String va = introspectedColumn.getJavaProperty("condition.") +
        // " != null";
        //            isNotNullElement.addAttribute(new Attribute("test", va)); //$NON-NLS-1$
    ...
    public boolean sqlMapSelectByExampleWithoutBLOBsElementGenerated(final XmlElement element, final IntrospectedTable introspectedTable) {
        /*
         * if (databaseType.contains("oracle")) { final XmlElement oracleHeadIncludeElement = new XmlElement("include");
         * oracleHeadIncludeElement.addAttribute(new Attribute("refid", "common.Oracle_Pagination_Head")); // 在第一个地方增加 element.addElement(0,
         * oracleHeadIncludeElement); final XmlElement oracleTailIncludeElement = new XmlElement("include"); oracleTailIncludeElement.addAttribute(new
         * Attribute("refid", "common.Oracle_Pagination_Tail")); // 在最后增加 element.addElement(element.getElements().size(), oracleTailIncludeElement);
         * } else if (databaseType.contains("mysql")) { final XmlElement mysqlLimitIncludeElement = new XmlElement("include");
         * mysqlLimitIncludeElement.addAttribute(new Attribute("refid", "common.Mysql_Pagination_Limit")); // 在最后增加
    ...
    public List<GeneratedXmlFile> contextGenerateAdditionalXmlFiles(final IntrospectedTable introspectedTable) {
        ...
        answer.addElement(getIfExampleConditonValid(introspectedTable));
        answer.addElement(getExampleWhereClause(introspectedTable, false));
        answer.addElement(getExampleWhereClause(introspectedTable, true));
        ...
        final GeneratedXmlFile gxf = new GeneratedXmlFile(document, properties.getProperty("fileName", "common_SqlMap.xml"), //$NON-NLS-1$ //$NON-NLS-2$
            context.getSqlMapGeneratorConfiguration().getTargetPackage(), context.getSqlMapGeneratorConfiguration().getTargetProject(), false);
    ...
    private XmlElement getIfExampleConditonValid(final IntrospectedTable introspectedTable) {
        final XmlElement answer = new XmlElement("sql"); //$NON-NLS-1$

        answer.addAttribute(new Attribute("id", "If_Example_Conditon_Valid")); //$NON-NLS-1$

        final XmlElement ifElement = new XmlElement("if"); //$NON-NLS-1$
        ifElement.addAttribute(new Attribute("test", "condition.valid")); //$NON-NLS-1$ //$NON-NLS-2$
        final XmlElement trimElement = new XmlElement("trim"); //$NON-NLS-1$
        ...
        ExampleWhereClauseElementGenerator exampleWhereClauseElementGenerator = new ExampleWhereClauseElementGenerator(false);
        trimElement.addElement(exampleWhereClauseElementGenerator.getMiddleForEachElement(null));

        for (final IntrospectedColumn introspectedColumn : introspectedTable.getNonBLOBColumns()) {
            if (stringHasValue(introspectedColumn.getTypeHandler())) {
                trimElement.addElement(exampleWhereClauseElementGenerator.getMiddleForEachElement(introspectedColumn));
            }
        }
    ...
    private XmlElement getExampleWhereClause(final IntrospectedTable introspectedTable, boolean isForUpdateByExample) {
        final XmlElement answer = new XmlElement("sql"); //$NON-NLS-1$

        if (isForUpdateByExample) {
            answer.addAttribute(new Attribute("id", "Update_By_Example_Where_Clause")); //$NON-NLS-1$
        } else {
            answer.addAttribute(new Attribute("id", "Example_Where_Clause")); //$NON-NLS-1$
        }

        context.getCommentGenerator().addComment(answer);

        final XmlElement whereElement = new XmlElement("where"); //$NON-NLS-1$
        answer.addElement(whereElement);

        final XmlElement outerForEachElement = new XmlElement("foreach"); //$NON-NLS-1$
        if (isForUpdateByExample) {
        ...
        XmlElement includeIfConditionElement = new XmlElement("include"); //$NON-NLS-1$
        includeIfConditionElement.addAttribute(new Attribute("refid", "common.If_Example_Conditon_Valid")); //$NON-NLS-1$
        outerForEachElement.addElement(includeIfConditionElement);
